...
class BotonController extends Controller
{
    ...
    {
        // Decodificar las cadenas JSON en arrays
        $request->merge([
            'contenido' => json_decode($request->contenido, true),
            'posicion_x' => json_decode($request->posicion_x, true),
            'posicion_y' => json_decode($request->posicion_y, true),
        ]);
...
// TODO se envian los botones con su info correctamente, pero no se envian a la base de datos, averiguar el porque